<template>
  <div :style="styleChange" @click="change" :class="{'avue-echart-img--rotate':rotate==true}">
    <img v-if="option.openValue===value && option.openUrl" :src="option.openUrl"/>
    <p :style="openStyle" v-else-if="option.openValue===value" :class="{active:classShow}" @mousedown="classShow=true">{{option.openText}}</p>
    <img v-if="option.closeValue===value && option.closeUrl" :src="option.closeUrl"/>
    <p :style="closeStyle" v-else-if="option.closeValue===value" :class="{active2:classShow}" @mousedown="classShow2=true">{{option.closeText}}</p>
  </div>
</template>
<script>
